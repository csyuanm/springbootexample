...
public class ThreadPoolManager implements ThreadPool {
	...
	private ThreadPoolManager(int workerNum2) {
		...
		for (int i = 0; i < workerNum; i++) {
			// 每个空间的对象初始化
			workThrads[i] = new WorkThread();  //使用内部类初始化线程
	...
	/**
	 * 实现thread pool，重写execute方法，单个任务
	 */
	@Override
	...
	/**
	 * 实现thread pool，重写execute方法，任务数组
	 */
	@Override
	...
	/**
	 * 实现thread pool，重写execute方法，任务列表
	 */
	@Override
